...
{
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_TIMEOUT, 6000); // 最大执行时间
    curl_setopt($ch, CURLOPT_CONNECTTIMEOUT_MS, 0); // 在尝试连接时等待的秒数
...
function de_weight($orders,$date){
    $status = [3,6,7];      // 成功订单的状态
    $success_order = [];
    $cancel_order = [];
    foreach ($orders as $order) {
        // 获取成功的订单
        if (in_array($order['order_status_id'],$status)){
            $success_order[] = [
                'customer_email_address' => strtolower($order['customer_email_address']),
            ...
            $cancel_order[strtolower($order['customer_email_address'])] = [
                'customer_email_address' => strtolower($order['customer_email_address']),
                'date_added' => $order['date_added'],
                'currency_code' => $order['currency_code'],
                'currency_value' => $order['currency_value'],
    ...
    // 成功金额
    $arr['success_money'] = 0;
    foreach ($success_order as $success_item){
        $arr['success_money'] += ($success_item['order_total']*$success_item['currency_value']);
        $key = $success_item['customer_email_address'];
        if (array_key_exists($key,$cancel_order)){
            unset($cancel_order[$key]);
        }
    }
    $cancel_count = count($cancel_order);
    $arr['success_count'] = count($success_order);      // 成功订单
    $arr['total_count'] = $cancel_count + $arr['success_count'];      // 全部订单
    $arr['pay_ratio'] = 0;
    $arr['date'] = $date;
    if ($arr['total_count'] != 0){
        $arr['pay_ratio'] = ($arr['success_count']/$arr['total_count'])*100;
    }
    return $arr;
}
