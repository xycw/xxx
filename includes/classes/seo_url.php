...
class seo_url {
	...
	public function __construct()
	{
		...
		if ($this->_attributes['URL_REWRITE_TYPE'] == 2) $this->generate_product_name();
		...
		switch ($main_page) {
			...
			case FILENAME_PRODUCT:
				if (isset($p2['pID'])) {
					$pID = $p2['pID'];
					unset($p2['pID']);
					$link .=  ($this->_attributes['URL_REWRITE_TYPE']==2?$this->_data['product'][$pID]:$this->product_name($pID) . '-p_' . $pID) . '.html';
				}
			break;
			default:
				$link .= $main_page . '.html';
	...
	public function product_name($pID)
	{
		global $db;
		if (!array_key_exists($pID, $this->_data['product'])) {
			$sql = "SELECT product_id, name, url, master_category_id FROM " . TABLE_PRODUCT . " WHERE product_id = " . $pID;
			$result = $db->Execute($sql, false, true, 604800);
			while (!$result->EOF) {
				if ($this->_attributes['URL_REWRITE_PRODUCT_LEVEL']==1) {
					$this->_data['product'][$result->fields['product_id']] = $this->_data['category'][$result->fields['master_category_id']] . '/' . (strlen($result->fields['url']) > 0 ? $result->fields['url']:$this->_strip($result->fields['name']));
