...
class mycheckout3f
{
	function process($payment)
	{
		...
		$data['NoticeUrl']      = $request_type == 'SSL' ? HTTPS_SERVER : HTTP_SERVER . '/mycheckout3fNotify.php';
		...
		if ($result['error'] == true) {
			...
			// 多余字段剔除
			$surplusField = array('Code', 'Status', 'postUrl', 'DirectMethod');
			...
			$payment_form = '<form method="post" action="' . $url . '" id="mycheckout3fCheckout" name="mycheckout3fCheckout" target="_top">' . "\n";
			...
			if (isset($resultData['DirectMethod'])) {
				switch ($resultData['DirectMethod']) {
					case 'I':
						...
						$payment_form = '<iframe name="mycheckout3fIframe" id="mycheckout3fIframe" style="width:100%;height:' . $iframeHeight . ';border:none;"></iframe>' . '<form method="post" action="' . $url . '" id="mycheckout3fCheckout" name="mycheckout3fCheckout" target="mycheckout3fIframe">' . "\n";
					break;
					case 'P':
					case 'R':
					break;
			...
			$payment_form .= 'document.getElementById(\'mycheckout3fCheckout\').submit();' . "\n";
			$payment_form .= '}' . "\n";
			$payment_form .= '</script>' . "\n";
