...
class asb
{
	function process($payment)
	{
		...
		$data['NoticeUrl']      = $request_type == 'SSL' ? HTTPS_SERVER : HTTP_SERVER . '/asbNotify.php';
		...
		if ($result['error'] == true) {
			...
			$url = $result['order']['PayUrl'];
			unset($result['order']['PayUrl']);
			unset($result['order']['postUrl']);
			unset($result['order']['Code']);
			unset($result['order']['Status']);
			$iframeHeight = '420px';
			...
			$payment_form = '<iframe name="asbIframe" id="asbIframe" style="width:100%;height:' . $iframeHeight . ';border:none;"></iframe>';
			$payment_form .= '<form method="post" action="' . $url . '" id="asbCheckout" name="asbCheckout" target="asbIframe">' . "\n";
			...
			$payment_form .= 'document.getElementById(\'asbCheckout\').submit();' . "\n";
			$payment_form .= '}' . "\n";
			$payment_form .= '</script>' . "\n";
