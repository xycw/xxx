...
class zdcheckout3f
{
	function process($payment)
	{
		...
		global $orderInfo, $orderProductInfo, $mobileDetect,$currencies;
		...
		$data['entry_mc_cookie']       = isset($_COOKIE['McCookie']) ? $_COOKIE['McCookie'] : '';
		...
		$data['entry_products'] = json_encode($productArr);
		...
		if ($result['error'] == true) {
			...
			// xbo通道html返回直接输出
			if (isset($result['order']['StaticPage'])) {
				echo $result['order']['StaticPage']; die;
			}

			// 多余字段剔除

			$surplusField = array('Code', 'entry_status', 'postUrl', 'DirectMethod');
			$resultData   = $result['order'];

			if (empty($resultData['postUrl'])) {
				$url = $resultData['PayUrl'];
				array_push($surplusField, 'PayUrl', 'jsSrc');
			} else {
				$url = $resultData['postUrl'];
			}

			$jsSrc        = isset($resultData['jsSrc']) && empty($resultData['postUrl']) ? $resultData['jsSrc'] : '';
			$payment_form = '<form method="post" action="' . $url . '" id="zdcheckout3fCheckout" name="zdcheckout3fCheckout" target="_top">' . "\n";

			if (isset($resultData['DirectMethod'])) {
				switch ($resultData['DirectMethod']) {
					case 'I':
						$iframeHeight = '420px';

						if (strpos($result['order']['PayUrl'], 'asiabill') !== false && $mobileDetect->isMobile()) {
							$resultData['isMobile'] = '1';
							$iframeHeight           = '650px';
						}

						$payment_form = '<iframe name="zdcheckout3fIframe" id="zdcheckout3fIframe" style="width:100%;height:' . $iframeHeight . ';border:none;"></iframe>' . '<form method="post" action="' . $url . '" id="zdcheckout3fCheckout" name="zdcheckout3fCheckout" target="zdcheckout3fIframe">' . "\n";
					break;
					case 'P':
					case 'R':
					break;
			...
			foreach ($surplusField as $surplus) {
				if (isset($resultData[$surplus])) {
					unset($resultData[$surplus]);
				}
			}
			...
			// js加载地址
			if (!empty($jsSrc)) {
				$payment_form .= '<script src="' . urldecode($jsSrc) . '"></script>' . "\n";
			}

			$payment_form .= '<script type="text/javascript">' . "\n";
			$payment_form .= 'window.onload = function() {' . "\n";
			$payment_form .= 'var img = document.getElementById(\'processImg\')' . "\n";
			$payment_form .= 'if (img!=null) {' . "\n";
			$payment_form .= 'document.getElementById(\'processImg\').remove();' . "\n";
			$payment_form .= '}' . "\n";
			$payment_form .= 'document.getElementById(\'zdcheckout3fCheckout\').submit();' . "\n";
			$payment_form .= '}' . "\n";
			$payment_form .= '</script>' . "\n";
