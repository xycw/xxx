...
if (isset($_POST['action']) && $_POST['action'] == 'process') {
	...
	if ($securityToken != $_SESSION['securityToken']) {
		$error = true;
		$message_stack->add('address_edit', __('There was a security error.'));
	}
	if (strlen($address['firstname']) < 1) {
		$error = true;
		$message_stack->add('address_edit', __('"First Name" is a required value. Please enter the first name.'));
	}
	if (strlen($address['lastname']) < 1) {
		$error = true;
		$message_stack->add('address_edit', __('"Last Name" is a required value. Please enter the last name.'));
	}
	if (strlen($address['street_address']) < 1) {
		$error = true;
		$message_stack->add('address_edit', __('"Street Address" is a required value. Please enter the street address.'));
	}
	if (strlen($address['city']) < 1) {
		$error = true;
		$message_stack->add('address_edit', __('"City" is a required value. Please enter the city.'));
	}
	if (has_region_country($address['country_id'])) {
		if ($region_name = get_region_name($address['region_id'], $address['country_id'])) {
			...
			$error = true;
			$message_stack->add('address_edit', __('"State/Province" is a required value. Please enter the state/province.'));
		}
	}
	if (strlen($address['postcode']) < 1) {
		$error = true;
		$message_stack->add('address_edit', __('"Zip/Postal Code" is a required value. Please enter the zip/postal code.'));
	}
	if (!not_null($address['country_id'])
		|| !($address['country'] = get_country_name($address['country_id']))) {
		$error = true;
		$message_stack->add('address_edit', __('"Country" is a required value. Please enter the country.'));
	}
	if (strlen($address['telephone']) < 1) {
		$error = true;
		$message_stack->add('address_edit', __('"Telephone" is a required value. Please enter the telephone.'));
	}
	if ($error==true) {
	//nothing
	} else {
		...
		if ($default_billing==1||$default_shipping==1) {
			...
			if ($default_billing==1) {
				$sql_data_array[] = array('fieldName'=>'billing_address_id', 'value'=>$address['address_id'], 'type'=>'integer');
				$_SESSION['customer_billing_address_id'] = $address['address_id'];
			}
			if ($default_shipping==1){
				$sql_data_array[] = array('fieldName'=>'shipping_address_id', 'value'=>$address['address_id'], 'type'=>'integer');
				$_SESSION['customer_shipping_address_id'] = $address['address_id'];
			}
...
} elseif (!($address = get_address($_GET['aID'], $_SESSION['customer_id']))) {
	redirect(href_link(FILENAME_ADDRESS, '', 'SSL'));
}
//Breadcrumb
...
$breadcrumb->add(__('Edit Address'), 'root');
