...
<?php
	...
	switch ($_option['type']) {
		...
		case 'select' :
			$optionValueHtml .= '<select class="' . $optionvalueRequired . '" name="attribute[' . $_option['product_option_id'] . ']" id="attribute_' . $_option['product_option_id'] . '">';
		...
		case 'text' :
			...
			$optionValuePriceHtml = $_optionValue['price'] > 0 ? ' ( ' . $_optionValue['price_prefix'] . $currencies->display_price($_optionValue['price']) . ' )' : '';
			$optionValueHtml .= '<input type="text" class="input-text ' . $optionvalueRequired . '" value="" name="attribute[' . $_option['product_option_id'] . ']" id="attribute_' . $_option['product_option_id'] . '" placeholder="' . '" /><br />' . $optionValuePriceHtml;
		break;
		case 'radio' :
			foreach ($_option['value'] as $_optionValue) {
				$optionValuePriceHtml = $_optionValue['price'] > 0 ? ' ( ' . $_optionValue['price_prefix'] . $currencies->display_price($_optionValue['price']) . ' )' : '';
				...
				$optionValueHtml .= '<label for="attribute_' . $_option['product_option_id'] . '_' . $_optionValue['product_option_value_id'] . '">' . __($_optionValue['name']) . $optionValuePriceHtml . '</label><br />';
			}
		break;
		case 'checkbox' :
			foreach ($_option['value'] as $_optionValue) {
...
?>
	<dl>
		...
		<dt><label class="required"><?php echo $_option['required'] ? '<em>*</em>' : ''; ?><?php echo __($_option['name']); ?></label></dt>
		<?php } ?>
