...
<div class="box-collateral box-also">
	<h2><?php echo __('You may also like'); ?></h2>
	<?php $column_count = ALSO_PURCHASED_PER_ROW; ?>
	<?php $i=0; ?>
	<?php foreach ($alsoPurchasedList as $_product) { ?>
	<?php if ($i++%$column_count==0) { ?>
    <ul class="products-also">
    <?php } ?>
    	<li class="item">
    		<a href="<?php echo href_link(FILENAME_PRODUCT, 'pID=' . $_product['product_id']); ?>" title="<?php echo $_product['nameAlt']; ?>" class="product-image"><img width="<?php echo ALSO_PURCHASED_IMAGE_WIDTH; ?>" height="<?php echo ALSO_PURCHASED_IMAGE_HEIGHT; ?>" alt="<?php echo $_product['nameAlt']; ?>" src="<?php echo get_small_image($_product['image'], ALSO_PURCHASED_IMAGE_WIDTH, ALSO_PURCHASED_IMAGE_HEIGHT); ?>" /></a>
    ...
    <?php if ($i%$column_count==0 || $i==$alsoPurchasedListCount) { ?>
    </ul>
    <?php } ?>
    <?php } ?>
    <script type="text/javascript">decorateList($('ul.products-also'));</script>
</div>
<?php } ?>
