...
<div class="recent_viewed">
	...
	<?php $column_count = RECENT_VIEWED_PER_ROW; ?>
	<?php $i=0; ?>
    ...
    <?php } ?>
    	<li class="item">
    		<a href="<?php echo href_link(FILENAME_PRODUCT, 'pID=' . $_product['product_id']); ?>" title="<?php echo $_product['nameAlt']; ?>" class="product-image"><img width="<?php echo RECENT_VIEWED_IMAGE_WIDTH; ?>" height="<?php echo RECENT_VIEWED_IMAGE_HEIGHT; ?>" alt="<?php echo $_product['nameAlt']; ?>" src="<?php echo get_small_image($_product['image'], RECENT_VIEWED_IMAGE_WIDTH, RECENT_VIEWED_IMAGE_HEIGHT); ?>" /></a>
    ...
    <?php if ($i%$column_count==0 || $i==$recentViewedListCount) { ?>
    </ul>
    <?php } ?>
    <?php } ?>
    <script type="text/javascript">decorateList($('.recent_viewed ul'));</script>
</div>
<?php } ?>
