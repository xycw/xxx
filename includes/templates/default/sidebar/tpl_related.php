<?php include(DIR_FS_CATALOG_MODULES . get_module_directory('sidebar/related.php')); ?>
<?php if (count($relatedSidebarList)>0) { ?>
<div class="block block-related">
	<div class="block-title">
        <strong><span><?php echo __('Related'); ?></span></strong>
    </div>
    <div class="block-content">
    	<ol id="related-items" class="mini-products-list">
    		<?php foreach ($relatedSidebarList as $_product) { ?>
    		<li class="item">
    			<a href="<?php echo href_link(FILENAME_PRODUCT, 'pID=' . $_product['product_id']); ?>" title="<?php echo $_product['nameAlt']; ?>" class="product-image"><img width="<?php echo RELATED_SIDEBAR_IMAGE_WIDTH; ?>" height="<?php echo RELATED_SIDEBAR_IMAGE_HEIGHT; ?>" alt="<?php echo $_product['nameAlt']; ?>" src="<?php echo get_small_image($_product['image'], RELATED_SIDEBAR_IMAGE_WIDTH, RELATED_SIDEBAR_IMAGE_HEIGHT); ?>" /></a>
    	...
    	<script type="text/javascript">decorateList($('#related-items'))</script>
    </div>
</div>
<?php } ?>
