<?php include(DIR_FS_CATALOG_MODULES . get_module_directory('sidebar/ordered_products.php')); ?>
<?php if (count($orderedProductsSidebarList)>0) { ?>
<div class="block block-ordered-products">
	<div class="block-title">
        <strong><span><?php echo __('Recent Shopping'); ?></span></strong>
    </div>
    <div class="block-content">
    	<ol id="ordered-products-items" class="mini-products-list">
    		<?php foreach ($orderedProductsSidebarList as $_product) { ?>
    		<li class="item">
    			<a href="<?php echo href_link(FILENAME_PRODUCT, 'pID=' . $_product['product_id']); ?>" title="<?php echo $_product['nameAlt']; ?>" class="product-image"><img width="<?php echo ORDERED_PRODUCTS_SIDEBAR_IMAGE_WIDTH; ?>" height="<?php echo ORDERED_PRODUCTS_SIDEBAR_IMAGE_HEIGHT; ?>" alt="<?php echo $_product['nameAlt']; ?>" src="<?php echo get_small_image($_product['image'], ORDERED_PRODUCTS_SIDEBAR_IMAGE_WIDTH, ORDERED_PRODUCTS_SIDEBAR_IMAGE_HEIGHT); ?>" /></a>
    	...
    	<script type="text/javascript">decorateList($('#ordered-products-items'))</script>
    </div>
</div>
<?php } ?>
