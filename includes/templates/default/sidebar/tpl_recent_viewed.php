<?php include(DIR_FS_CATALOG_MODULES . get_module_directory('sidebar/recent_viewed.php')); ?>
<?php if (count($recentViewedSidebarList)>0) { ?>
<div class="block block-recent_viewed">
	<div class="block-title">
        <strong><span><?php echo __('Recent Viewed'); ?></span></strong>
    </div>
    <div class="block-content">
    	<ol id="recent_viewed-items" class="mini-products-list">
    		<?php foreach ($recentViewedSidebarList as $_product) { ?>
    		<li class="item">
    			<a href="<?php echo href_link(FILENAME_PRODUCT, 'pID=' . $_product['product_id']); ?>" title="<?php echo $_product['nameAlt']; ?>" class="product-image"><img width="<?php echo RECENT_VIEWED_SIDEBAR_IMAGE_WIDTH; ?>" height="<?php echo RECENT_VIEWED_SIDEBAR_IMAGE_HEIGHT; ?>" alt="<?php echo $_product['nameAlt']; ?>" src="<?php echo get_small_image($_product['image'], RECENT_VIEWED_SIDEBAR_IMAGE_WIDTH, RECENT_VIEWED_SIDEBAR_IMAGE_HEIGHT); ?>" /></a>
    	...
    	<script type="text/javascript">decorateList($('#recent_viewed-items'))</script>
    </div>
</div>
<?php } ?>
